...
def patch_tokenizer(tokenizer: "PreTrainedTokenizerBase") -> None:
    if "PreTrainedTokenizerBase" not in str(tokenizer._pad.__func__):
        tokenizer._pad = MethodType(PreTrainedTokenizerBase._pad, tokenizer)
    if tokenizer.eos_token_id is None:
        tokenizer.eos_token = "<|endoftext|>"
        logging.info(f"Add eos token: {tokenizer.eos_token}")
    if tokenizer.pad_token is None:
        if tokenizer.unk_token_id is not None:
            tokenizer.pad_token = tokenizer.unk_token
        else:
            tokenizer.pad_token = tokenizer.eos_token
        logging.info(f"Add pad token: {tokenizer.pad_token}")
...
def patch_config(
    ...
    if getattr(config, "torch_dtype", None) == 'qwen':
...
def _configure_rope(config: "PretrainedConfig", rope_scaling: str = None) -> None:
    r"""
    Configures the RoPE(Rotatory position encoding) scaling strategy for the model.
    :param config:
    :param rope_scaling: The scaling strategy to use.
    :return:
    """
    if not hasattr(config, "rope_scaling"):
        logging.warning("Current model does not support RoPE scaling.")
        return
    scaling_factor = 2.0
    setattr(config, "rope_scaling", {"type": rope_scaling, "factor": scaling_factor})
    logging.info(f"Using {rope_scaling} scaling strategy and setting scaling factor to {scaling_factor}.")


def _configure_flashattn(config_kwargs: Dict[str, Any]) -> None:
    if not is_flash_attn2_available():
        logging.warning("FlashAttention2 is not installed.")
        return

    config_kwargs["use_flash_attention_2"] = True
    logging.info("Using FlashAttention-2 for faster and inference.")
...
def _configure_quantization(
    ...
    if load_in_8bits:
        ...
        logging.info("Quantizing model to 8 bit.")
        ...
        logging.info("Quantizing model to 4 bit.")
